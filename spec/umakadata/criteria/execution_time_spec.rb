...
SPARQL
        ...
        it 'should return not nil' do
          allow(target).to receive(:response_time).with(Umakadata::Criteria::ExecutionTime::BASE_QUERY, nil)
            .and_return(1000)
          allow(target).to receive(:response_time).with(Umakadata::Criteria::ExecutionTime::TARGET_QUERY, nil)
            .and_return(10000)
        ...
        describe '#response_time' do
          it 'should return nil when query is malformed' do
            ...
            expect(target.response_time(MALFORMED_QUERY, nil)).to eq nil
          end
          ...
          it 'should return time when query is correctly' do
           ...
           expect(target.response_time(Umakadata::Criteria::ExecutionTime::BASE_QUERY, nil).instance_of?(Float)).to be true
           expect(target.response_time(Umakadata::Criteria::ExecutionTime::TARGET_QUERY, nil).instance_of?(Float)).to be true
         end
       end
